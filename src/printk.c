...
#define SLOG_FMT    (ELOG_FMT_LVL|ELOG_FMT_TAG|ELOG_FMT_TIME)
void BUG(void)
{
...
static char log_buf[CONSOLEBUF_SIZE] = { 0 };
void pr_arr(const void *arr, size_t len)
{
    ...
    while (len)
    {
        ...
        console_write_bytes(log_buf, strlen(log_buf));
        console_write_bytes("\r\n", 2);
        len -= _len;
        idx += _len;
    }
...
#endif
static inline int32_t divide(long *n, long base)
{
    int32_t res;
...
void printk(const char *fmt, ...)
{
     ...
     * length. */
    length = os_vsnprintf(log_buf, sizeof(log_buf) - 1, fmt, args);
    if (length > sizeof(log_buf) - 1)
        length = sizeof(log_buf) - 1;
    console_write_bytes(log_buf, length);
    va_end(args);
}
#endif
...
int setup_print(void)
{
    ...
    elog_set_fmt(ELOG_LVL_ASSERT, SLOG_FMT);
    elog_set_fmt(ELOG_LVL_ERROR,  SLOG_FMT);
    elog_set_fmt(ELOG_LVL_WARN,   SLOG_FMT);
    elog_set_fmt(ELOG_LVL_INFO,   SLOG_FMT);
    elog_set_fmt(ELOG_LVL_DEBUG,  SLOG_FMT);

    elog_set_category(MODULE_OS,  ELOG_LVL_DEBUG, "OS");
    elog_set_category(MODULE_UART, ELOG_LVL_DEBUG, "UART");
    elog_set_category(MODULE_APP, ELOG_LVL_DEBUG, "APP");
